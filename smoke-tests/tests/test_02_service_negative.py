"""
test_02_service_negative.py — Feature Gates & Negative Tests
...
differently on the configured SKU.
"""
...
class TestScalingRejected:
    ...
    def test_neg_01_reject_replica_count(self, rest):
        """NEG-01: Setting replicaCount > 1 should be rejected on this SKU."""
        ...
        if resp.status_code == 200:
            ...
            assert actual == 1, f"replicaCount was accepted as {actual} — unexpected for this SKU"
        else:
            # Any 4xx/5xx counts as rejected — currently returns 500
            assert resp.status_code >= 400, f"Unexpected status {resp.status_code}"

    def test_neg_02_reject_partition_count(self, rest):
        """NEG-02: Setting partitionCount > 1 should be rejected on this SKU."""
        body = {"properties": {"partitionCount": 2}}
        resp = rest.mgmt_patch(body=body)
        if resp.status_code == 200:
            data = resp.json()
            actual = data.get("properties", {}).get("partitionCount", 1)
            assert actual == 1, f"partitionCount was accepted as {actual} — unexpected for this SKU"
        else:
            # Any 4xx/5xx counts as rejected — currently returns 500
            assert resp.status_code >= 400, f"Unexpected status {resp.status_code}"
...
class TestInvalidCreation:
    ...
    def test_neg_04_invalid_location(self, rest, search_sku):
        """NEG-04: Create service in unsupported region returns error."""
        url = (
            f"https://management.azure.com/subscriptions/{rest.subscription_id}"
            f"/resourceGroups/{rest.resource_group}"
            f"/providers/Microsoft.Search/searchServices/smoke-bad-region"
        )
        body = {"location": "antarcticaland", "sku": {"name": search_sku}}
        resp = rest.mgmt_request("PUT", url, body)
        # Expect 400 or similar error
        assert resp.status_code >= 400, f"Expected error for invalid location, got {resp.status_code}"

    def test_neg_05_duplicate_service_name(self, rest, search_location, search_sku):
        """NEG-05: Create service with existing name is idempotent or 409."""
        url = rest.mgmt_url()
        body = {"location": search_location, "sku": {"name": search_sku}}
        resp = rest.mgmt_request("PUT", url, body)
...
class TestServiceLimits:
    ...
    def test_neg_08_delete_disposable_service(self, rest, disposable_service_name, search_location, search_sku):
...
class TestNetworkSecurityFeatureGates:
    ...
    def test_neg_09_ip_firewall_rules(self, rest):
        """NEG-09: Discover whether IP firewall rules work on this SKU."""
        body = {
            "properties": {
                "networkRuleSet": {
                    "ipRules": [
    ...
    def test_neg_10_private_endpoint(self, rest):
        """NEG-10: Discover whether shared private link works on this SKU."""
    ...
    def test_neg_11_cmk_encryption(self, rest):
        """NEG-11: Discover whether CMK encryption works on this SKU."""
        resp = rest.mgmt_get()
        assert_status(resp, 200)
        data = resp.json()
        props = data.get("properties", {})
        encryption = props.get("encryptionWithCmk", {})
        # Service should report CMK enforcement status
    ...
    def test_neg_12_disable_public_access(self, rest):
        """NEG-12: Discover whether publicNetworkAccess can be disabled on this SKU."""
...
class TestCMKEncryption:
    ...
    def test_neg_13_enable_identity_on_service(self, rest):
        """NEG-13: Enable system-assigned managed identity on the service."""
        body = {"identity": {"type": "SystemAssigned"}}
        resp = rest.mgmt_patch(body=body)
        # Service may or may not support MSI — record the result
        if resp.status_code == 200:
            identity = resp.json().get("identity", {})
            principal_id = identity.get("principalId", "")
            assert principal_id, "MSI enabled but no principalId returned"
        # If 400, this SKU doesn't support MSI — that's the discovery

    def test_neg_14_create_index_with_cmk(self, rest, cmk_config):
        """NEG-14: Create an index with CMK encryptionKey on this SKU."""
        ...
        # CMK may or may not work on this SKU — record behavior
    ...
    def test_neg_15_create_synonym_map_with_cmk(self, rest, cmk_config):
        """NEG-15: Create a synonym map with CMK encryptionKey on this SKU."""
        if cmk_config is None:
            pytest.skip("CMK not configured")
